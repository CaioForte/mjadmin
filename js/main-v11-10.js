...
function renderProducts(){
  ...
  renderProductStats(); renderStock(); renderDashboardLowStock(); if(typeof renderDashboardLiveData==='function')renderDashboardLiveData();
}
...
function renderDashboardLiveData(){
  ...
  const paidToday=sales.filter(v=>v.status==='Pago'&&v.date===today);
  const revenue=paidToday.reduce((sum,v)=>sum+Number(v.total||0),0);
  const expensesToday=expenses.filter(e=>e.status==='Pago'&&e.date===today).reduce((sum,e)=>sum+Number(e.value||0),0);
  // Lucro estimado: receita do dia menos custo dos itens vendidos e despesas pagas do dia.
  const productCost=paidToday.reduce((sum,v)=>sum+(v.items||[]).reduce((ss,i)=>{
    const p=products.find(x=>x.id===i.productId);
    return ss + (p ? Number(p.cost||0)*Number(i.qty||0) : 0);
  },0),0);
  const profit=Math.max(0,revenue-productCost-expensesToday);
  ...
  const body=document.getElementById('dashboardRecentSalesBody');
  if(body){
    const recent=[...sales].sort((a,b)=>String(b.createdAt||b.date||'').localeCompare(String(a.createdAt||a.date||''))).slice(0,5);
    body.innerHTML=recent.map(v=>`<tr><td>${esc(v.num||'-')}</td><td>${esc(saleClientName(v))}</td><td>${moneyBR(v.total)}</td><td>${esc(v.payment||'-')}</td><td><span class="status ${saleStatusClass(v.status)}">${esc(v.status||'-')}</span></td></tr>`).join('')||'<tr><td colspan="5" class="empty-table">Nenhuma venda registrada.</td></tr>';
  }
}
...
// V11.10 - Dashboard e estoque exibem somente dados reais do Google Sheets/cache.
try{renderDashboardLowStock();renderDashboardLiveData();}catch(e){console.warn('[MJ V11.10] Dashboard inicial:',e);}
