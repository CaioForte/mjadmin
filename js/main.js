...
function canAccess(view){
  if(MJCurrentUser.role==='Administrador') return true;
  if(view==='configuracoes') return true; // Minha senha fica disponível para todos.
  return MJUserPermissions.has(view);
}
// V11.23 - relatórios gerenciais + InfinitePay pausada
function showProcessOverlay(title, message){
  let overlay=document.getElementById('processOverlay');
  if(!overlay){
    overlay=document.createElement('div');
    overlay.id='processOverlay';
    overlay.className='process-overlay';
    overlay.innerHTML=`<div class="process-card" role="status" aria-live="polite"><div class="process-spinner" aria-hidden="true"></div><h2 id="processOverlayTitle"></h2><p id="processOverlayMessage"></p><small>Não feche esta janela nem clique novamente enquanto o processo é concluído.</small></div>`;
    document.body.appendChild(overlay);
  }
  document.getElementById('processOverlayTitle').textContent=title||'Processando...';
  document.getElementById('processOverlayMessage').textContent=message||'Aguarde alguns instantes.';
  overlay.classList.add('show');
  document.body.classList.add('process-busy');
}
function updateProcessOverlay(message){const el=document.getElementById('processOverlayMessage');if(el&&message)el.textContent=message;}
...
function go(view,options={}){
  ...
  // V12.0.6: Vendas e Nova Venda sempre consultam novamente o Supabase.
  // Isso evita trabalhar com a lista de produtos/clientes/vendas que ficou
  // carregada em memória antes de um cadastro recente.
  if(view==='vendas'||view==='nova-venda')setTimeout(()=>refreshSalesModuleData(view),0);
  if(view==='compras')setTimeout(()=>refreshPurchasesModuleData(),0);
...
function quoteDetailHTML(q){
 ...
 <div class="client-modal-actions quote-modal-actions"><button class="btn btn-line" onclick="printQuote('${q.id}')">🖨 Imprimir / PDF</button><button class="btn btn-line quote-email-btn" onclick="emailQuote('${q.id}')">✉ Enviar por e-mail</button><button class="btn btn-line quote-share-btn" onclick="shareQuote('${q.id}')">↗ Compartilhar</button><button class="btn btn-line" onclick="duplicateQuote('${q.id}')">Duplicar</button>${q.status!=='Convertido'?`<button class="btn btn-primary" onclick="convertQuote('${q.id}')">Converter em venda</button>`:''}</div>`;
}
function quoteShareText(q){
 const client=clients.find(c=>c.id===q.clientId);
 const lines=[
  `ORÇAMENTO ${q.num} - MJ ENVELOPAMENTO`,
  `Cliente: ${quoteClientName(q)}`,
  `Emissão: ${brDate(q.date)}`,
  `Validade: ${brDate(q.validity)}`,
  '',
  'ITENS:'
 ];
 (q.items||[]).forEach(i=>lines.push(`• ${i.name} — ${Number(i.qty).toLocaleString('pt-BR')} x ${moneyBR(i.unitPrice)} = ${moneyBR(Number(i.qty)*Number(i.unitPrice))}`));
 lines.push('',`Subtotal: ${moneyBR(q.subtotal)}`);
 if(Number(q.discount||0)>0)lines.push(`Desconto: ${moneyBR(q.discount)}`);
 lines.push(`TOTAL: ${moneyBR(q.total)}`);
 if(q.paymentTerms)lines.push('',`Condição de pagamento: ${q.paymentTerms}`);
 if(q.obs)lines.push('',`Observações: ${q.obs}`);
 return lines.join('\n');
}
async function emailQuote(id){
 ...
 if(!email){toast('Este cliente não possui e-mail cadastrado. Edite o cadastro do cliente e informe um e-mail.');return;}
 if(!window.MJCloud?.sendQuoteEmail){toast('Atualize o Apps Script para habilitar o envio de orçamento por e-mail.');return;}
 const btn=document.querySelector('#quoteModal .quote-email-btn');
 const original=btn?.innerHTML||'';
 try{
  if(btn){btn.disabled=true;btn.classList.add('is-sending');btn.innerHTML='<span class="mini-spinner"></span> Enviando...';}
  const res=await window.MJCloud.sendQuoteEmail(id);
  const destino=res?.data?.email||email;
  toast(`Orçamento enviado para ${destino}.`);
 }catch(err){
  console.error('[MJ] Envio do orçamento:',err);
  toast(err?.message||'Não foi possível enviar o orçamento por e-mail.');
 }finally{
  if(btn){btn.disabled=false;btn.classList.remove('is-sending');btn.innerHTML=original||'✉ Enviar por e-mail';}
 }
}
async function shareQuote(id){
 const q=quotes.find(x=>x.id===id);if(!q)return;
 const text=`Olá, ${quoteClientName(q)}!\n\nSegue seu orçamento:\n\n${quoteShareText(q)}`;
 const data={title:`Orçamento ${q.num} - MJ Envelopamento`,text};
 try{
  if(navigator.share){await navigator.share(data);return;}
 }catch(err){
  if(err?.name==='AbortError')return;
  console.warn('[MJ] Compartilhamento nativo indisponível:',err);
 }
 // Fallback ideal para desktop: abre WhatsApp Web/App com a mensagem pronta.
 const url=`https://wa.me/?text=${encodeURIComponent(text)}`;
 const w=window.open(url,'_blank','noopener,noreferrer');
 if(!w)toast('Não foi possível abrir o compartilhamento. Verifique o bloqueio de pop-ups do navegador.');
}
...
window.viewQuote=viewQuote;window.editQuote=editQuote;window.deleteQuote=deleteQuote;window.convertQuote=convertQuote;window.duplicateQuote=duplicateQuote;window.printQuote=printQuote;window.emailQuote=emailQuote;window.shareQuote=shareQuote;window.updateQuoteItem=updateQuoteItem;window.removeQuoteItem=removeQuoteItem;window.closeQuoteModal=closeQuoteModal;
...
let saleReceiptDraft=null;
let purchaseReceiptDraft=null;
const RECEIPT_MAX_BYTES=8*1024*1024;
const RECEIPT_ALLOWED_TYPES=['application/pdf','image/jpeg','image/png','image/webp'];
function validateReceiptFile(file){
  if(!file)return '';
  if(!RECEIPT_ALLOWED_TYPES.includes(String(file.type||'').toLowerCase()))return 'Formato não permitido. Use PDF, JPG, PNG ou WEBP.';
  if(Number(file.size||0)>RECEIPT_MAX_BYTES)return 'O arquivo deve ter no máximo 8 MB.';
  return '';
}
function fileToBase64(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||'').split(',').pop()||'');r.onerror=()=>reject(new Error('Não foi possível ler o arquivo selecionado.'));r.readAsDataURL(file);});}
async function uploadReceiptForRecord(entityType,id,file){
  const err=validateReceiptFile(file);if(err)throw new Error(err);
  const base64=await fileToBase64(file);
  const response=await window.MJCloud.uploadReceipt(entityType,id,file.name,file.type,base64);
  return response.data||{};
}
function receiptLabel(file){return file?`${file.name} • ${(file.size/1024/1024).toLocaleString('pt-BR',{maximumFractionDigits:2})} MB`:'Nenhum arquivo selecionado';}
function bindReceiptInput(inputId,labelId,type){
  const input=document.getElementById(inputId),label=document.getElementById(labelId);if(!input)return;
  input.addEventListener('change',()=>{const file=input.files?.[0]||null;const err=validateReceiptFile(file);if(err){toast(err);input.value='';if(type==='sale')saleReceiptDraft=null;else purchaseReceiptDraft=null;if(label)label.textContent='Nenhum arquivo selecionado';return;}if(type==='sale')saleReceiptDraft=file;else purchaseReceiptDraft=file;if(label)label.textContent=receiptLabel(file);});
}
let receiptViewerObjectUrl='';
function closeReceiptViewer(){
 const modal=document.getElementById('receiptViewerModal');
 if(modal)modal.classList.remove('show');
 if(receiptViewerObjectUrl){URL.revokeObjectURL(receiptViewerObjectUrl);receiptViewerObjectUrl='';}
 const content=document.getElementById('receiptViewerContent');if(content)content.innerHTML='';
}
function ensureReceiptViewer(){
 let modal=document.getElementById('receiptViewerModal');if(modal)return modal;
 modal=document.createElement('div');modal.id='receiptViewerModal';modal.className='receipt-viewer-modal';
 modal.innerHTML=`<div class="receipt-viewer-box"><div class="receipt-viewer-head"><div><small>COMPROVANTE</small><b id="receiptViewerName">Carregando...</b></div><button class="receipt-viewer-close" type="button" aria-label="Fechar">×</button></div><div id="receiptViewerContent" class="receipt-viewer-content"><div class="receipt-viewer-loading"><span class="process-spinner"></span><b>Carregando comprovante...</b><small>Buscando o arquivo privado no Google Drive.</small></div></div><div class="receipt-viewer-actions"><button id="receiptViewerNewTab" class="btn btn-line" type="button" disabled>Abrir em nova aba</button><button class="btn btn-primary receipt-viewer-close-btn" type="button">Fechar</button></div></div>`;
 document.body.appendChild(modal);
 modal.querySelector('.receipt-viewer-close')?.addEventListener('click',closeReceiptViewer);
 modal.querySelector('.receipt-viewer-close-btn')?.addEventListener('click',closeReceiptViewer);
 modal.addEventListener('click',e=>{if(e.target===modal)closeReceiptViewer();});
 return modal;
}
async function viewReceipt(entityType,id){
 const modal=ensureReceiptViewer(),content=document.getElementById('receiptViewerContent'),name=document.getElementById('receiptViewerName'),tab=document.getElementById('receiptViewerNewTab');
 if(receiptViewerObjectUrl){URL.revokeObjectURL(receiptViewerObjectUrl);receiptViewerObjectUrl='';}
 name.textContent='Carregando...';
 content.innerHTML='<div class="receipt-viewer-loading"><span class="process-spinner"></span><b>Carregando comprovante...</b><small>Buscando o arquivo privado no Google Drive.</small></div>';
 tab.disabled=true;tab.onclick=null;modal.classList.add('show');
 try{
  const res=await window.MJCloud.getReceipt(entityType,id),data=res.data||{};
  if(!data.base64)throw new Error('O arquivo não foi recebido.');
  const raw=atob(data.base64),bytes=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)bytes[i]=raw.charCodeAt(i);
  const blob=new Blob([bytes],{type:data.mimeType||'application/octet-stream'});receiptViewerObjectUrl=URL.createObjectURL(blob);name.textContent=data.fileName||'Comprovante';
  const mime=String(data.mimeType||'').toLowerCase();
  if(mime==='application/pdf')content.innerHTML=`<iframe class="receipt-viewer-frame" src="${receiptViewerObjectUrl}" title="Comprovante"></iframe>`;
  else if(mime.startsWith('image/'))content.innerHTML=`<img class="receipt-viewer-image" src="${receiptViewerObjectUrl}" alt="Comprovante">`;
  else content.innerHTML='<div class="receipt-viewer-error"><b>Visualização não disponível para este formato.</b><small>Use “Abrir em nova aba”.</small></div>';
  tab.disabled=false;tab.onclick=()=>window.open(receiptViewerObjectUrl,'_blank','noopener');
 }catch(e){console.error('[MJ] Visualização comprovante:',e);name.textContent='Não foi possível abrir';content.innerHTML=`<div class="receipt-viewer-error"><b>Não foi possível carregar o comprovante.</b><small>${esc(e.message||'Erro ao buscar o arquivo.')}</small></div>`;toast(e.message||'Não foi possível abrir o comprovante.');}
}
function chooseReceiptForRecord(entityType,id){
  ...
  input.addEventListener('change',async()=>{const file=input.files?.[0];if(!file){input.remove();return;}const err=validateReceiptFile(file);if(err){toast(err);input.remove();return;}toast('Enviando comprovante...');try{const meta=await uploadReceiptForRecord(entityType,id,file);if(entityType==='Venda'){const rec=sales.find(x=>String(x.id)===String(id));if(rec)Object.assign(rec,meta);localStorage.setItem('mj_sales',JSON.stringify(sales));renderSales();viewSale(id);}else{const rec=purchases.find(x=>String(x.id)===String(id));if(rec)Object.assign(rec,meta);savePurchasesLocal();renderPurchases();viewPurchase(id);}toast('Comprovante anexado com sucesso.');}catch(e){console.error('[MJ] Comprovante:',e);toast(e.message||'Não foi possível anexar o comprovante.');}finally{input.remove();}});input.click();
}
...
let __salesModuleRefreshPromise=null;
async function refreshSalesModuleData(view){
  if(!window.MJCloud?.refreshSalesContext)return;
  if(__salesModuleRefreshPromise)return __salesModuleRefreshPromise;

  __salesModuleRefreshPromise=(async()=>{
    try{
      const fresh=await window.MJCloud.refreshSalesContext();
      if(!fresh?.ok)return;

      if(Array.isArray(fresh.products))products=fresh.products;
      if(Array.isArray(fresh.clients))clients=fresh.clients;
      if(Array.isArray(fresh.sales))sales=fresh.sales;
      ...
      // Mantém a seleção atual da tela e apenas atualiza os dados necessários.
      if(view==='nova-venda'){
        const select=document.getElementById('saleProductSelect');
        if(select&&!select.value)select.selectedIndex=0;
      }
    }catch(err){
      console.error('[MJ Supabase] Atualização de Vendas/Nova Venda:',err);
      toast('Não foi possível atualizar os dados de vendas agora.');
    }finally{
      __salesModuleRefreshPromise=null;
    }
  })();

  return __salesModuleRefreshPromise;
}
...
async function finishSale(){
 ...
 const receiptFile=saleReceiptDraft;
 ...
 showProcessOverlay('Finalizando venda...',receiptFile?'Salvando a venda e enviando o comprovante para o Google Drive...':'Salvando venda, estoque e financeiro...');
 const installments=isIP?1:Math.max(1,Number(document.getElementById('saleInstallments')?.value||1));const firstDueDate=document.getElementById('saleFirstDueDate')?.value||document.getElementById('saleDate').value||currentISODate();const sale={id:'s'+Date.now(),num,date:document.getElementById('saleDate').value||currentISODate(),clientId:document.getElementById('saleClient').value,clientName:document.getElementById('saleClient').value?'':'Cliente Balcão',seller:document.getElementById('saleSeller').value.trim(),items:saleItemsDraft.map(i=>({...i})),subtotal:calc.subtotal,discount:calc.discount,total:calc.total,payment,status,installments,firstDueDate,obs:document.getElementById('saleObs').value.trim(),sourceQuoteId:saleSourceQuoteId,createdAt:new Date().toISOString()};
 ...
 const saleTxs=buildInstallmentTransactions({type:'Entrada',source:'Venda',sourceId:sale.id,description:`Venda ${num} - ${saleClientName(sale)}`,category:'Vendas',date:sale.date,dueDate:firstDueDate,value:sale.total,payment,status:status==='Pago'?'Pago':'Pendente',installments,notes:isIP?'Aguardando confirmação da InfinitePay.':'Lançamento gerado automaticamente pela venda.'});financialTransactions.unshift(...saleTxs);saveFinancial();
 ...
 let receiptFailed='';if(receiptFile){updateProcessOverlay('Venda registrada. Enviando o comprovante para o Google Drive...');try{await window.MJCloud.pushKey('mj_sales',sales);const meta=await uploadReceiptForRecord('Venda',sale.id,receiptFile);Object.assign(sale,meta);localStorage.setItem('mj_sales',JSON.stringify(sales));}catch(e){console.error('[MJ] Comprovante venda:',e);receiptFailed=e.message||'Falha ao anexar comprovante.';}}
 localStorage.removeItem('mj_sale_draft');renderSales();renderProducts();renderStock();renderFinancialSummary();clearSaleForm(false);
 if(isIP){updateProcessOverlay('Venda registrada. Gerando checkout InfinitePay...');toast(`${num} criada. Gerando checkout InfinitePay...`);await createInfinitePayCheckoutForSale(sale.id,true);hideProcessOverlay();return;}
 hideProcessOverlay();toast(receiptFailed?`${num} finalizada, mas o comprovante não foi anexado: ${receiptFailed}`:`${num} finalizada com sucesso.`);go('vendas');
}
...
function saleDetailHTML(s){const ip=s.infinitePay||{};return `<div class="quote-detail-head"><div><small>Cliente</small><h4>${esc(saleClientName(s))}</h4><p>Vendedor: ${esc(s.seller||'-')}</p></div><span class="status ${saleStatusClass(s.status)}">${esc(s.status)}</span></div><div class="quote-detail-meta"><div><small>Data</small><b>${brDate(s.date)}</b></div><div><small>Pagamento</small><b>${esc(s.payment)}</b></div><div><small>Parcelamento</small><b>${Number(s.installments||1)}x</b></div><div><small>Origem</small><b>${s.sourceQuoteId?'Orçamento convertido':'Venda direta'}</b></div>${s.payment==='InfinitePay'?`<div><small>Order NSU</small><b>${esc(ip.orderNsu||s.num)}</b></div><div><small>Transaction NSU</small><b>${esc(ip.transactionNsu||'Aguardando')}</b></div><div><small>Método InfinitePay</small><b>${esc(ipMethodLabel(ip.captureMethod))}</b></div>`:''}</div><div class="table-scroll"><table class="data-table"><thead><tr><th>Item</th><th>Qtd.</th><th>Unitário</th><th>Subtotal</th></tr></thead><tbody>${(Array.isArray(s.items)?s.items:[]).map(i=>`<tr><td>${esc(i.name)}</td><td>${Number(i.qty).toLocaleString('pt-BR')}</td><td>${moneyBR(i.unitPrice)}</td><td><b>${moneyBR(Number(i.qty)*Number(i.unitPrice))}</b></td></tr>`).join('')}</tbody></table></div><div class="quote-detail-footer"><div class="quote-detail-obs"><small>Observação</small><p>${esc(s.obs||'Sem observações.')}</p></div><div class="quote-totals compact"><div class="summary-row"><span>Subtotal</span><b>${moneyBR(s.subtotal)}</b></div><div class="summary-row"><span>Desconto</span><b>${moneyBR(s.discount)}</b></div><div class="summary-row total"><span>Total</span><span>${moneyBR(s.total)}</span></div></div></div>${s.receiptUrl?`<div class="receipt-detail-card"><span>📎</span><div><small>Comprovante anexado</small><b>${esc(s.receiptFileName||'Arquivo')}</b></div><button class="btn btn-line btn-small" onclick="viewReceipt('Venda','${s.id}')">Abrir</button></div>`:''}<div class="client-modal-actions"><button class="btn btn-line" onclick="printSale('${s.id}')">Imprimir</button>${s.receiptUrl?`<button class="btn btn-soft" onclick="chooseReceiptForRecord('Venda','${s.id}')">📎 Substituir comprovante</button>`:`<button class="btn btn-soft" onclick="chooseReceiptForRecord('Venda','${s.id}')">📎 Anexar comprovante</button>`}${s.payment==='InfinitePay'&&s.status!=='Pago'?`<button class="btn btn-soft" onclick="openInfinitePayCheckout('${s.id}')">∞ Abrir checkout</button><button class="btn btn-line" onclick="verifyInfinitePayPayment('${s.id}')">Verificar pagamento</button>`:''}${s.payment==='InfinitePay'&&s.status==='Pago'&&ip.receiptUrl?`<button class="btn btn-soft" onclick="openInfinitePayReceipt('${s.id}')">Ver comprovante</button>`:''}${s.status!=='Cancelado'?`<button class="btn btn-line danger-text" onclick="cancelSale('${s.id}')">Cancelar venda</button>`:''}</div>`;}
...
async function executeSaleCancellation(){
 const s=sales.find(x=>x.id===pendingSaleCancelId);if(!s)return closeSaleCancelConfirm();
 const btn=document.getElementById('confirmSaleCancelBtn');if(btn){btn.disabled=true;btn.textContent='Cancelando...';}
 try{
   const response=await window.MJCloud.cancelSale(s.id),data=response.data||{};
   if(data.venda)sales=sales.map(x=>x.id===s.id?data.venda:x);else sales=sales.map(x=>x.id===s.id?{...x,status:'Cancelado',cancelledAt:new Date().toISOString()}:x);
   if(Array.isArray(data.products))products=data.products;if(Array.isArray(data.financial))financialTransactions=data.financial;
   localStorage.setItem('mj_sales',JSON.stringify(sales));localStorage.setItem('mj_products',JSON.stringify(products));localStorage.setItem('mj_financial_transactions',JSON.stringify(financialTransactions));
   const num=s.num;closeSaleCancelConfirm();closeSaleModal();renderSales();renderProducts();renderStock();renderFinancialSummary();go('vendas');
   const text=document.getElementById('saleCancelSuccessText');if(text)text.innerHTML=`A venda <strong>${esc(num)}</strong> foi cancelada.`;
   const success=document.getElementById('saleCancelSuccessModal');if(success){success.classList.add('show');success.setAttribute('aria-hidden','false');}
 }catch(err){console.error('[MJ] Cancelamento venda:',err);toast(err.message||'Não foi possível cancelar a venda.');}
 finally{if(btn){btn.disabled=false;btn.textContent='Sim, cancelar venda';}}
}
...
function initSales(){document.getElementById('salesBody')?.addEventListener('click',handleSaleTableAction);bindReceiptInput('saleReceiptFile','saleReceiptFileName','sale');populateSaleClients();populateSaleProducts();document.getElementById('saleDate').value=currentISODate();if(document.getElementById('saleFirstDueDate'))document.getElementById('saleFirstDueDate').value=currentISODate();document.getElementById('saleNumberPreview').textContent=nextSaleNumber();document.getElementById('saleProductSelect')?.addEventListener('change',e=>{const p=products.find(x=>x.id===e.target.value);const input=document.getElementById('saleItemPrice');if(!input)return;input.value=p&&!isVariablePriceProduct(p)?Number(p.price||0).toLocaleString('pt-BR',{minimumFractionDigits:2}):'';input.placeholder=p&&isVariablePriceProduct(p)?'Informe o valor manualmente':'0,00';if(p&&isVariablePriceProduct(p))setTimeout(()=>input.focus(),0);});document.getElementById('addSaleItemBtn')?.addEventListener('click',addSaleItem);document.getElementById('saleDiscount')?.addEventListener('input',saleCalc);document.getElementById('saleInstallments')?.addEventListener('change',e=>{if(Number(e.target.value)>1){document.getElementById('saleStatus').value='Pendente';toast('Venda parcelada será criada como pendente para baixa parcela a parcela.');}});document.getElementById('finishSaleBtn')?.addEventListener('click',finishSale);document.getElementById('clearSaleBtn')?.addEventListener('click',()=>clearSaleForm(true));document.getElementById('newSaleBtn')?.addEventListener('click',()=>{clearSaleForm(false);go('nova-venda')});document.getElementById('saleSearch')?.addEventListener('input',renderSales);document.getElementById('saleStatusFilter')?.addEventListener('change',renderSales);document.getElementById('saleDateFilter')?.addEventListener('change',renderSales);document.getElementById('clearSaleFilters')?.addEventListener('click',()=>{document.getElementById('saleSearch').value='';document.getElementById('saleStatusFilter').value='';document.getElementById('saleDateFilter').value='';renderSales();});document.querySelectorAll('[data-close-sale-modal]').forEach(el=>el.addEventListener('click',closeSaleModal));renderSales();if(!loadSaleDraft())renderSaleDraft();renderFinancialSummary();}
window.updateSaleItem=updateSaleItem;window.removeSaleItem=removeSaleItem;window.viewSale=viewSale;window.printSale=printSale;window.cancelSale=cancelSale;window.closeSaleModal=closeSaleModal;window.loadSaleDraft=loadSaleDraft;window.viewReceipt=viewReceipt;window.closeReceiptViewer=closeReceiptViewer;window.chooseReceiptForRecord=chooseReceiptForRecord;
initSales();


// =====================================================
// V11.18 - PARCELAMENTO / PREVISTO X REALIZADO
// =====================================================
function addMonthsISO(dateStr,months){const d=new Date(`${dateStr||currentISODate()}T12:00:00`);const day=d.getDate();d.setDate(1);d.setMonth(d.getMonth()+Number(months||0));const last=new Date(d.getFullYear(),d.getMonth()+1,0).getDate();d.setDate(Math.min(day,last));return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;}
function splitInstallmentValues(total,count){count=Math.max(1,Number(count)||1);const cents=Math.round(Number(total||0)*100),base=Math.floor(cents/count),rest=cents-base*count;return Array.from({length:count},(_,i)=>(base+(i<rest?1:0))/100);}
function displayFinanceStatus(t){const st=txStatus(t);if(st==='Pendente'&&isOverdue(t.dueDate,st))return 'Vencido';if(st==='Pago')return t.type==='Entrada'?'Recebido':'Pago';return st;}
function buildInstallmentTransactions({type,source,sourceId,description,category,date,dueDate,value,payment,status,notes,installments=1,createdAt}){const count=Math.max(1,Number(installments)||1),values=splitInstallmentValues(value,count),groupId=`grp-${sourceId||Date.now()}-${Date.now()}`;return values.map((v,i)=>({id:`ft-${sourceId||'manual'}-${Date.now()}-${i+1}`,type,source,sourceId:sourceId||'',description:count>1?`${description} • Parcela ${i+1}/${count}`:description,category,date:date||currentISODate(),dueDate:addMonthsISO(dueDate||date||currentISODate(),i),value:v,payment:payment||'Outro',status:status||'Pendente',notes:notes||'',installmentNumber:i+1,installmentTotal:count,installmentGroupId:groupId,createdAt:createdAt||new Date().toISOString(),paidAt:(status==='Pago'?currentISODate():'')}));}
...
function financeStatusClass(s){return (s==='Pago'||s==='Recebido')?'paid':s==='Vencido'?'overdue':'pending';}
function txStatus(t){return t.status||'Pago';}
function normalizeFinancialTransactions(){
  financialTransactions=financialTransactions.map((t,i)=>({...t,id:t.id||`ft-mig-${Date.now()}-${i}`,type:t.type==='Saída'?'Saída':'Entrada',source:t.source||'Manual',sourceId:t.sourceId||'',description:t.description||'Lançamento',category:t.category||'Outros',date:t.date||currentISODate(),dueDate:t.dueDate||t.date||currentISODate(),value:Number(t.value)||0,payment:t.payment||'Outro',status:t.status||'Pago',notes:t.notes||'',installmentNumber:Number(t.installmentNumber||1),installmentTotal:Number(t.installmentTotal||1),createdAt:t.createdAt||new Date().toISOString()}));
...
function syncSaleFromTransaction(t){
  ...
  const related=financialTransactions.filter(x=>x.source==='Venda'&&x.sourceId===t.sourceId);const sale=sales.find(x=>x.id===t.sourceId);if(!sale)return;
  sale.status=related.length&&related.every(x=>txStatus(x)==='Pago')?'Pago':'Pendente';saveSales();renderSales();
}
...
function renderFinancialSummary(){
  ...
  const incList=paid.filter(t=>t.type==='Entrada'&&String(t.paidAt||t.date).startsWith(month));
  const expList=paid.filter(t=>t.type==='Saída'&&String(t.paidAt||t.date).startsWith(month));
  ...
  const recValue=rec.reduce((a,t)=>a+Number(t.value),0),payValue=pay.reduce((a,t)=>a+Number(t.value),0),forecast=bal+recValue-payValue,overdue=[...rec,...pay].filter(t=>isOverdue(t.dueDate,txStatus(t))),overdueValue=overdue.reduce((a,t)=>a+Number(t.value),0);set('financeReceivable',moneyBR(recValue));set('financePayable',moneyBR(payValue));set('financeReceivableCount',`${rec.length} pendente(s)`);set('financePayableCount',`${pay.length} pendente(s)`);set('financeForecastBalance',moneyBR(forecast));set('financeOverdueValue',moneyBR(overdueValue));set('financeOverdueCount',`${overdue.length} conta(s)`);
  const be=document.getElementById('financeBalance');if(be)be.classList.toggle('negative',bal<0);const fb=document.getElementById('financeForecastBalance');if(fb)fb.classList.toggle('negative',forecast<0);
  renderFinanceBars();
}
...
function renderFinanceBars(){
  ...
  const vals=keys.map(k=>({k,inc:financialTransactions.filter(t=>t.type==='Entrada'&&txStatus(t)==='Pago'&&String(t.paidAt||t.date).startsWith(k)).reduce((s,t)=>s+Number(t.value),0),out:financialTransactions.filter(t=>t.type==='Saída'&&txStatus(t)==='Pago'&&String(t.paidAt||t.date).startsWith(k)).reduce((s,t)=>s+Number(t.value),0)}));
...
function renderFinance(){
  ...
  const list=[...financialTransactions].sort((a,b)=>String(b.dueDate||b.date).localeCompare(String(a.dueDate||a.date))).filter(t=>{const hay=[t.description,t.source,t.category,t.payment].join(' ').toLowerCase(),display=displayFinanceStatus(t);const statusOk=!status||(status==='Pago'?(txStatus(t)==='Pago'):(status==='Vencido'?display==='Vencido':txStatus(t)===status));return(!q||hay.includes(q))&&(!type||t.type===type)&&statusOk&&(!month||String(t.dueDate||t.date).startsWith(month));});
  body.innerHTML=list.map(t=>{const display=displayFinanceStatus(t),parcel=Number(t.installmentTotal||1)>1?`<span class="finance-parcel-badge">${Number(t.installmentNumber||1)}/${Number(t.installmentTotal)}</span>`:'';return `<tr><td>${brDate(t.date)}<span class="finance-due-date">Venc. ${brDate(t.dueDate||t.date)}</span></td><td><b>${esc(t.description)}</b>${t.category?`<span class="quote-item-code">${esc(t.category)}</span>`:''}${parcel}</td><td><div class="finance-origin"><b>${esc(t.source)}</b><small>${t.sourceId?'Automático / vinculado':'Manual'}</small></div></td><td><span class="finance-type ${t.type==='Entrada'?'income':'expense'}">${esc(t.type)}</span></td><td><b class="${t.type==='Entrada'?'value-income':'value-expense'}">${t.type==='Entrada'?'+ ':'- '}${moneyBR(t.value)}</b></td><td>${esc(t.payment||'-')}</td><td><span class="status ${financeStatusClass(display)}">${esc(display)}</span></td><td><div class="quote-actions">${txStatus(t)==='Pendente'?`<button class="table-action success" onclick="markFinancialPaid('${t.id}')">${t.type==='Entrada'?'Receber':'Pagar'}</button>`:''}<button class="table-action" onclick="editFinancial('${t.id}')">Editar</button>${t.source==='Manual'?`<button class="table-action danger" onclick="deleteFinancial('${t.id}')">Excluir</button>`:''}</div></td></tr>`;}).join('')||'<tr><td colspan="8"><div class="empty-note">Nenhum lançamento encontrado.</div></td></tr>';
...
function markFinancialPaid(id){const t=financialTransactions.find(x=>x.id===id);if(!t)return;t.status='Pago';t.paidAt=currentISODate();saveFinancial();syncSaleFromTransaction(t);syncExpenseFromTransaction(t);renderFinance();toast(t.type==='Entrada'?'Conta recebida.':'Conta paga.');}
...
// =====================================================
// MJ ADMIN V11.17 - FORNECEDORES + COMPRAS / ENTRADA
// =====================================================
const defaultSuppliers=[];
const defaultPurchases=[];
let suppliers=JSON.parse(localStorage.getItem('mj_suppliers')||'null')||defaultSuppliers;
let purchases=JSON.parse(localStorage.getItem('mj_purchases')||'null')||defaultPurchases;
let purchaseItemsDraft=[];
let pendingPurchaseCancelId='';

function saveSuppliers(){
  localStorage.setItem('mj_suppliers',JSON.stringify(suppliers));
  window.MJCloud?.pushKey('mj_suppliers',suppliers).catch(err=>console.error('[MJ Cloud] Fornecedores:',err));
}
function savePurchasesLocal(){localStorage.setItem('mj_purchases',JSON.stringify(purchases));}
function purchaseSupplierName(p){const s=suppliers.find(x=>x.id===p.supplierId);return s?.name||p.supplierName||'Fornecedor não informado';}
function nextPurchaseNumber(){const nums=purchases.map(p=>parseInt(String(p.num||'').replace(/\D/g,''),10)).filter(Number.isFinite);return `COM-${String((nums.length?Math.max(...nums):0)+1).padStart(5,'0')}`;}
function purchaseStatusClass(s){return s==='Ativa'?'paid':'rejected';}
function purchaseFinanceState(p){
  if(p.generateFinance===false)return 'Sem financeiro';
  const txs=financialTransactions.filter(t=>t.source==='Compra'&&String(t.sourceId)===String(p.id));
  if(!txs.length)return p.financeStatus||'Pago';return txs.every(t=>txStatus(t)==='Pago')?'Pago':'Pendente';
}
function refreshSupplierDatalists(){
  const opts=suppliers.filter(s=>s.status==='Ativo').map(s=>`<option value="${esc(s.name)}"></option>`).join('');
  const a=document.getElementById('supplierNameList'),b=document.getElementById('supplierExpenseList');if(a)a.innerHTML=opts;if(b)b.innerHTML=opts;
}
function renderSupplierStats(){
  const month=currentISODate().slice(0,7),monthPurchases=purchases.filter(p=>p.status!=='Cancelada'&&String(p.date||'').startsWith(month));
  const set=(id,val)=>{const el=document.getElementById(id);if(el)el.textContent=val};
  set('supplierCount',suppliers.length);set('supplierActiveCount',suppliers.filter(s=>s.status==='Ativo').length);set('supplierPurchaseMonthCount',monthPurchases.length);set('supplierPurchaseMonthValue',moneyBR(monthPurchases.reduce((a,p)=>a+Number(p.total||0),0)));
}
function renderSuppliers(){
  const body=document.getElementById('suppliersBody');if(!body)return;
  const q=(document.getElementById('supplierSearch')?.value||'').toLowerCase().trim(),status=document.getElementById('supplierStatusFilter')?.value||'';
  const list=suppliers.filter(s=>{const hay=[s.name,s.document,s.phone,s.whatsapp,s.email,s.contact].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!status||s.status===status);}).sort((a,b)=>String(a.name).localeCompare(String(b.name),'pt-BR'));
  body.innerHTML=list.map(s=>`<tr><td><b>${esc(s.name)}</b>${s.contact?`<span class="quote-item-code">Contato: ${esc(s.contact)}</span>`:''}</td><td>${esc(s.document||'-')}</td><td>${esc(s.whatsapp||s.phone||'-')}</td><td>${esc(s.email||'-')}</td><td><span class="status ${s.status==='Ativo'?'paid':'rejected'}">${esc(s.status||'Ativo')}</span></td><td><div class="quote-actions"><button class="table-action" type="button" onclick="editSupplier('${s.id}')">Editar</button><button class="table-action danger" type="button" onclick="deleteSupplier('${s.id}')">Excluir</button></div></td></tr>`).join('')||'<tr><td colspan="6"><div class="empty-note">Nenhum fornecedor cadastrado.</div></td></tr>';
  const rc=document.getElementById('supplierResultCount');if(rc)rc.textContent=`${list.length} fornecedor(es)`;
  renderSupplierStats();refreshSupplierDatalists();populatePurchaseSuppliers();
}
function openSupplierForm(s=null){
  const panel=document.getElementById('supplierFormPanel');if(!panel)return;panel.hidden=false;document.getElementById('supplierForm').reset();
  document.getElementById('supplierEditId').value=s?.id||'';document.getElementById('supplierFormTitle').textContent=s?'Editar fornecedor':'Novo fornecedor';
  document.getElementById('supplierName').value=s?.name||'';document.getElementById('supplierDocument').value=s?.document||'';document.getElementById('supplierPhone').value=s?.phone||'';document.getElementById('supplierWhatsapp').value=s?.whatsapp||'';document.getElementById('supplierEmail').value=s?.email||'';document.getElementById('supplierContact').value=s?.contact||'';document.getElementById('supplierStatus').value=s?.status||'Ativo';document.getElementById('supplierNotes').value=s?.notes||'';
  panel.scrollIntoView({behavior:'smooth',block:'start'});setTimeout(()=>document.getElementById('supplierName')?.focus(),200);
}
function closeSupplierForm(){const p=document.getElementById('supplierFormPanel');if(p)p.hidden=true;}
function editSupplier(id){const s=suppliers.find(x=>x.id===id);if(s)openSupplierForm(s);}
function deleteSupplier(id){
  const s=suppliers.find(x=>x.id===id);if(!s)return;
  if(purchases.some(p=>p.supplierId===id)){toast('Este fornecedor possui compras vinculadas. Altere o status para Inativo em vez de excluir.');return;}
  suppliers=suppliers.filter(x=>x.id!==id);saveSuppliers();renderSuppliers();toast('Fornecedor excluído.');
}
function initSuppliers(){
  document.getElementById('newSupplierBtn')?.addEventListener('click',()=>openSupplierForm());document.getElementById('closeSupplierBtn')?.addEventListener('click',closeSupplierForm);document.getElementById('cancelSupplierBtn')?.addEventListener('click',closeSupplierForm);
  document.getElementById('supplierSearch')?.addEventListener('input',renderSuppliers);document.getElementById('supplierStatusFilter')?.addEventListener('change',renderSuppliers);
  document.getElementById('supplierForm')?.addEventListener('submit',e=>{e.preventDefault();const id=document.getElementById('supplierEditId').value,old=id?suppliers.find(x=>x.id===id):null;const name=document.getElementById('supplierName').value.trim();if(!name){toast('Informe o nome do fornecedor.');return;}const item={id:id||`forn-${Date.now()}`,name,document:document.getElementById('supplierDocument').value.trim(),phone:document.getElementById('supplierPhone').value.trim(),whatsapp:document.getElementById('supplierWhatsapp').value.trim(),email:document.getElementById('supplierEmail').value.trim(),contact:document.getElementById('supplierContact').value.trim(),status:document.getElementById('supplierStatus').value,notes:document.getElementById('supplierNotes').value.trim(),createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};if(id)suppliers=suppliers.map(x=>x.id===id?item:x);else suppliers.unshift(item);saveSuppliers();closeSupplierForm();renderSuppliers();toast(id?'Fornecedor atualizado.':'Fornecedor cadastrado.');});
  renderSuppliers();
}

function populatePurchaseSuppliers(){const el=document.getElementById('purchaseSupplier');if(!el)return;const cur=el.value;el.innerHTML='<option value="">Selecione um fornecedor</option>'+suppliers.filter(s=>s.status==='Ativo').map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('');if([...el.options].some(o=>o.value===cur))el.value=cur;}
function populatePurchaseProducts(){const el=document.getElementById('purchaseProductSelect');if(!el)return;el.innerHTML='<option value="">Selecione um produto</option>'+products.filter(p=>p.status==='Ativo'&&isStockTracked(p)).map(p=>`<option value="${esc(p.id)}">${esc(p.code)} • ${esc(p.name)} — estoque ${Number(p.stock||0).toLocaleString('pt-BR')}</option>`).join('');}
function purchaseCalc(){const subtotal=purchaseItemsDraft.reduce((s,i)=>s+Number(i.qty||0)*Number(i.unitCost||0),0),discount=Math.max(0,parseMoneyBR(document.getElementById('purchaseDiscount')?.value||0)),freight=Math.max(0,parseMoneyBR(document.getElementById('purchaseFreight')?.value||0)),total=Math.max(0,subtotal-Math.min(discount,subtotal)+freight);const a=document.getElementById('purchaseSubtotal'),b=document.getElementById('purchaseTotal');if(a)a.textContent=moneyBR(subtotal);if(b)b.textContent=moneyBR(total);return{subtotal,discount:Math.min(discount,subtotal),freight,total};}
function renderPurchaseDraft(){const body=document.getElementById('purchaseItemsBody');if(!body)return;body.innerHTML=purchaseItemsDraft.map(i=>`<tr><td><b>${esc(i.name)}</b><span class="quote-item-code">${esc(i.code||'')}</span></td><td><input class="quote-table-input qty" type="number" min="0.01" step="0.01" value="${Number(i.qty)}" onchange="updatePurchaseItem('${i.id}','qty',this.value)"></td><td><input class="quote-table-input price" value="${Number(i.unitCost).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}" onchange="updatePurchaseItem('${i.id}','cost',this.value)"></td><td><b>${moneyBR(Number(i.qty)*Number(i.unitCost))}</b></td><td><button class="action-btn delete" type="button" onclick="removePurchaseItem('${i.id}')">×</button></td></tr>`).join('')||'<tr><td colspan="5" class="empty-table">Adicione os produtos recebidos nesta compra.</td></tr>';purchaseCalc();}
function updatePurchaseItem(id,field,value){const i=purchaseItemsDraft.find(x=>x.id===id);if(!i)return;if(field==='qty')i.qty=Math.max(.01,Number(value)||.01);if(field==='cost')i.unitCost=Math.max(0,parseMoneyBR(value));renderPurchaseDraft();}
function removePurchaseItem(id){purchaseItemsDraft=purchaseItemsDraft.filter(i=>i.id!==id);renderPurchaseDraft();}
function addPurchaseItem(){const pid=document.getElementById('purchaseProductSelect')?.value,p=products.find(x=>x.id===pid);if(!p){toast('Selecione um produto.');return;}const qty=Number(document.getElementById('purchaseItemQty')?.value||0),raw=String(document.getElementById('purchaseItemCost')?.value||'').trim();if(qty<=0){toast('Informe uma quantidade válida.');return;}if(!raw){toast('Informe o custo unitário.');return;}const cost=Math.max(0,parseMoneyBR(raw)),existing=purchaseItemsDraft.find(i=>i.productId===pid&&Number(i.unitCost)===cost);if(existing)existing.qty+=qty;else purchaseItemsDraft.push({id:`pi-${Date.now()}-${Math.random().toString(36).slice(2,6)}`,productId:p.id,code:p.code,name:p.name,qty,unitCost:cost});document.getElementById('purchaseProductSelect').value='';document.getElementById('purchaseItemQty').value='1';document.getElementById('purchaseItemCost').value='';renderPurchaseDraft();}
function openPurchaseForm(){const p=document.getElementById('purchaseFormPanel');if(!p)return;purchaseItemsDraft=[];purchaseReceiptDraft=null;const rf=document.getElementById('purchaseReceiptFile');if(rf)rf.value='';const rl=document.getElementById('purchaseReceiptFileName');if(rl)rl.textContent='Nenhum arquivo selecionado';p.hidden=false;document.getElementById('purchaseNumber').value=nextPurchaseNumber();document.getElementById('purchaseDate').value=currentISODate();document.getElementById('purchaseDueDate').value=currentISODate();document.getElementById('purchaseSupplier').value='';document.getElementById('purchasePayment').value='PIX';document.getElementById('purchaseFinanceStatus').value='Pago';document.getElementById('purchaseInstallments').value='1';document.getElementById('purchaseGenerateFinance').checked=true;document.getElementById('purchaseUpdateCost').checked=true;document.getElementById('purchaseNotes').value='';document.getElementById('purchaseDiscount').value='0,00';document.getElementById('purchaseFreight').value='0,00';populatePurchaseSuppliers();populatePurchaseProducts();renderPurchaseDraft();p.scrollIntoView({behavior:'smooth',block:'start'});}
function closePurchaseForm(){const p=document.getElementById('purchaseFormPanel');if(p)p.hidden=true;purchaseItemsDraft=[];purchaseReceiptDraft=null;const rf=document.getElementById('purchaseReceiptFile');if(rf)rf.value='';const rl=document.getElementById('purchaseReceiptFileName');if(rl)rl.textContent='Nenhum arquivo selecionado';}
function renderPurchaseStats(){const month=currentISODate().slice(0,7),active=purchases.filter(p=>p.status!=='Cancelada'),monthList=active.filter(p=>String(p.date||'').startsWith(month)),pending=active.filter(p=>purchaseFinanceState(p)==='Pendente');const set=(id,val)=>{const el=document.getElementById(id);if(el)el.textContent=val};set('purchaseMonthCount',monthList.length);set('purchaseMonthValue',moneyBR(monthList.reduce((s,p)=>s+Number(p.total||0),0)));set('purchasePendingValue',moneyBR(pending.reduce((s,p)=>s+Number(p.total||0),0)));set('purchaseTotalCount',purchases.length);renderSupplierStats();}
function renderPurchases(){const body=document.getElementById('purchasesBody');if(!body)return;const q=(document.getElementById('purchaseSearch')?.value||'').toLowerCase().trim(),status=document.getElementById('purchaseStatusFilter')?.value||'',finance=document.getElementById('purchaseFinanceFilter')?.value||'',month=document.getElementById('purchaseMonthFilter')?.value||'';const list=[...purchases].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).filter(p=>{const fs=purchaseFinanceState(p);return(!q||[p.num,purchaseSupplierName(p)].join(' ').toLowerCase().includes(q))&&(!status||p.status===status)&&(!finance||fs===finance)&&(!month||String(p.date||'').startsWith(month));});body.innerHTML=list.map(p=>{const fs=purchaseFinanceState(p);return `<tr><td><b>${esc(p.num)}</b></td><td>${brDate(p.date)}</td><td>${esc(purchaseSupplierName(p))}</td><td><b>${moneyBR(p.total)}</b></td><td>${esc(p.payment||'-')}</td><td><span class="status ${fs==='Pago'?'paid':fs==='Pendente'?'pending':'converted'}">${esc(fs)}</span></td><td><span class="status ${purchaseStatusClass(p.status)}">${esc(p.status||'Ativa')}</span></td><td><div class="sale-row-actions"><button class="table-action" type="button" onclick="viewPurchase('${p.id}')">◉ Ver</button><button class="table-action" type="button" onclick="printPurchase('${p.id}')">▣ Imprimir</button>${p.status!=='Cancelada'?`<button class="table-action danger" type="button" onclick="requestCancelPurchase('${p.id}')">✕ Cancelar</button>`:''}</div></td></tr>`;}).join('')||'<tr><td colspan="8"><div class="empty-note">Nenhuma compra registrada.</div></td></tr>';const rc=document.getElementById('purchaseResultCount');if(rc)rc.textContent=`${list.length} compra(s)`;renderPurchaseStats();}
function purchaseDetailHTML(p){return `<div class="quote-detail-head"><div><small>Fornecedor</small><h4>${esc(purchaseSupplierName(p))}</h4><p>${brDate(p.date)} • ${esc(p.payment||'Não informado')}</p></div><span class="status ${purchaseStatusClass(p.status)}">${esc(p.status)}</span></div><div class="quote-detail-meta"><div><small>Vencimento</small><b>${brDate(p.dueDate)}</b></div><div><small>Financeiro</small><b>${esc(purchaseFinanceState(p))}</b></div><div><small>Parcelas</small><b>${Number(p.installments||1)}x</b></div><div><small>Atualizou custo</small><b>${p.updateCost!==false?'Sim':'Não'}</b></div></div><div class="table-scroll"><table class="data-table"><thead><tr><th>Produto</th><th>Qtd.</th><th>Custo</th><th>Total</th></tr></thead><tbody>${(p.items||[]).map(i=>`<tr><td>${esc(i.name)}</td><td>${Number(i.qty).toLocaleString('pt-BR')}</td><td>${moneyBR(i.unitCost)}</td><td><b>${moneyBR(Number(i.qty)*Number(i.unitCost))}</b></td></tr>`).join('')}</tbody></table></div><div class="quote-detail-footer"><div class="quote-detail-obs">${p.notes?`<small>Observações</small><p>${esc(p.notes)}</p>`:'<small>Sem observações adicionais.</small>'}</div><div class="quote-totals compact"><div class="summary-row"><span>Subtotal</span><b>${moneyBR(p.subtotal)}</b></div><div class="summary-row"><span>Desconto</span><b>${moneyBR(p.discount)}</b></div><div class="summary-row"><span>Frete</span><b>${moneyBR(p.freight)}</b></div><div class="summary-row total"><span>Total</span><span>${moneyBR(p.total)}</span></div></div></div>${p.receiptUrl?`<div class="receipt-detail-card"><span>📎</span><div><small>Comprovante anexado</small><b>${esc(p.receiptFileName||'Arquivo')}</b></div><button class="btn btn-line btn-small" onclick="viewReceipt('Compra','${p.id}')">Abrir</button></div>`:''}<div class="client-modal-actions quote-modal-actions"><button class="btn btn-line" onclick="printPurchase('${p.id}')">🖨 Imprimir</button>${p.receiptUrl?`<button class="btn btn-soft" onclick="chooseReceiptForRecord('Compra','${p.id}')">📎 Substituir comprovante</button>`:`<button class="btn btn-soft" onclick="chooseReceiptForRecord('Compra','${p.id}')">📎 Anexar comprovante</button>`}${p.status!=='Cancelada'?`<button class="btn btn-danger" onclick="requestCancelPurchase('${p.id}')">Cancelar compra</button>`:''}</div>`;}
function viewPurchase(id){const p=purchases.find(x=>x.id===id);if(!p)return;document.getElementById('purchaseModalTitle').textContent=p.num;document.getElementById('purchaseModalSubtitle').textContent=`Compra registrada em ${brDate(p.date)}`;document.getElementById('purchaseModalContent').innerHTML=purchaseDetailHTML(p);const m=document.getElementById('purchaseModal');m.classList.add('show');m.setAttribute('aria-hidden','false');}
function closePurchaseModal(){const m=document.getElementById('purchaseModal');if(m){m.classList.remove('show');m.setAttribute('aria-hidden','true');}}
function printPurchase(id){const p=purchases.find(x=>x.id===id);if(!p)return;const w=window.open('','_blank');if(!w){toast('O navegador bloqueou a janela de impressão.');return;}w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(p.num)}</title><style>body{font-family:Arial;color:#251d3a;margin:40px}header{display:flex;justify-content:space-between;border-bottom:3px solid #6a20ff;padding-bottom:18px}.brand{font-size:26px;font-weight:900;color:#6a20ff}.muted{color:#777;font-size:12px}table{width:100%;border-collapse:collapse;margin:22px 0}th,td{padding:10px;border-bottom:1px solid #ddd;text-align:left}th{background:#f5f2ff}.totals{margin-left:auto;width:300px}.row{display:flex;justify-content:space-between;padding:7px}.total{font-size:20px;border-top:2px solid #6a20ff}@media print{body{margin:18mm}}</style></head><body><header><div><div class="brand">MJ ENVELOPAMENTO</div><div class="muted">Entrada de estoque / compra</div></div><div style="text-align:right"><b>${esc(p.num)}</b><div class="muted">${brDate(p.date)}</div></div></header><h2>REGISTRO DE COMPRA</h2><p><b>Fornecedor:</b> ${esc(purchaseSupplierName(p))}</p><table><thead><tr><th>Produto</th><th>Qtd.</th><th>Custo unit.</th><th>Total</th></tr></thead><tbody>${(p.items||[]).map(i=>`<tr><td>${esc(i.name)}</td><td>${Number(i.qty).toLocaleString('pt-BR')}</td><td>${moneyBR(i.unitCost)}</td><td>${moneyBR(Number(i.qty)*Number(i.unitCost))}</td></tr>`).join('')}</tbody></table><div class="totals"><div class="row"><span>Subtotal</span><b>${moneyBR(p.subtotal)}</b></div><div class="row"><span>Desconto</span><b>${moneyBR(p.discount)}</b></div><div class="row"><span>Frete</span><b>${moneyBR(p.freight)}</b></div><div class="row total"><span>Total</span><b>${moneyBR(p.total)}</b></div></div>${p.notes?`<p><b>Observações:</b> ${esc(p.notes)}</p>`:''}<script>window.onload=()=>window.print()<\/script></body></html>`);w.document.close();}
async function finalizePurchase(){const receiptFile=purchaseReceiptDraft;const supplierId=document.getElementById('purchaseSupplier')?.value,supplier=suppliers.find(s=>s.id===supplierId);if(!supplier){toast('Selecione o fornecedor.');return;}if(!purchaseItemsDraft.length){toast('Adicione pelo menos um produto à compra.');return;}const calc=purchaseCalc();if(parseMoneyBR(document.getElementById('purchaseDiscount').value)>calc.subtotal){toast('O desconto não pode ser maior que o subtotal.');return;}const btn=document.getElementById('savePurchaseBtn');if(btn){btn.disabled=true;btn.textContent='Registrando...';}showProcessOverlay('Finalizando compra...',receiptFile?'Salvando a compra e enviando o comprovante para o Google Drive...':'Salvando compra, estoque e financeiro...');const compra={id:`cmp-${Date.now()}`,num:document.getElementById('purchaseNumber').value||nextPurchaseNumber(),date:document.getElementById('purchaseDate').value||currentISODate(),supplierId,supplierName:supplier.name,items:purchaseItemsDraft.map(i=>({...i})),subtotal:calc.subtotal,discount:calc.discount,freight:calc.freight,total:calc.total,payment:document.getElementById('purchasePayment').value,financeStatus:document.getElementById('purchaseFinanceStatus').value,installments:Math.max(1,Number(document.getElementById('purchaseInstallments')?.value||1)),generateFinance:document.getElementById('purchaseGenerateFinance').checked,updateCost:document.getElementById('purchaseUpdateCost').checked,dueDate:document.getElementById('purchaseDueDate').value||document.getElementById('purchaseDate').value||currentISODate(),notes:document.getElementById('purchaseNotes').value.trim(),status:'Ativa',createdAt:new Date().toISOString()};try{const response=await window.MJCloud.finalizePurchase(compra),data=response.data||{};const saved=data.compra||compra;if(!purchases.some(p=>p.id===saved.id))purchases.unshift(saved);if(Array.isArray(data.products))products=data.products;if(Array.isArray(data.financial))financialTransactions=data.financial;let receiptFailed='';if(receiptFile){updateProcessOverlay('Compra registrada. Enviando o comprovante para o Google Drive...');try{const meta=await uploadReceiptForRecord('Compra',saved.id,receiptFile);Object.assign(saved,meta);const local=purchases.find(x=>x.id===saved.id);if(local&&local!==saved)Object.assign(local,meta);}catch(e){console.error('[MJ] Comprovante compra:',e);receiptFailed=e.message||'Falha ao anexar comprovante.';}}savePurchasesLocal();localStorage.setItem('mj_products',JSON.stringify(products));localStorage.setItem('mj_financial_transactions',JSON.stringify(financialTransactions));closePurchaseForm();renderPurchases();renderProducts();renderStock();renderFinance();renderDashboardLowStock();populatePurchaseProducts();toast(receiptFailed?`${saved.num} registrada, mas o comprovante não foi anexado: ${receiptFailed}`:`${saved.num} registrada. Estoque atualizado.`);go('compras');}catch(err){console.error('[MJ] Compra:',err);toast(err.message||'Não foi possível registrar a compra.');}finally{hideProcessOverlay();if(btn){btn.disabled=false;btn.textContent='✓ Registrar compra e entrada';}}}
function requestCancelPurchase(id){const p=purchases.find(x=>x.id===id);if(!p||p.status==='Cancelada')return;pendingPurchaseCancelId=id;const sum=document.getElementById('confirmPurchaseCancelSummary');if(sum)sum.innerHTML=`<strong>${esc(p.num)}</strong> · ${esc(purchaseSupplierName(p))} · ${moneyBR(p.total)}`;const m=document.getElementById('confirmPurchaseCancelModal');if(m){m.classList.add('show');m.setAttribute('aria-hidden','false');}}
function closePurchaseCancelConfirm(){pendingPurchaseCancelId='';const m=document.getElementById('confirmPurchaseCancelModal');if(m){m.classList.remove('show');m.setAttribute('aria-hidden','true');}}
async function executePurchaseCancellation(){const id=pendingPurchaseCancelId,p=purchases.find(x=>x.id===id);if(!p)return closePurchaseCancelConfirm();const btn=document.getElementById('confirmPurchaseCancelBtn');if(btn){btn.disabled=true;btn.textContent='Cancelando...';}try{const response=await window.MJCloud.cancelPurchase(id),data=response.data||{};if(data.compra)purchases=purchases.map(x=>x.id===id?data.compra:x);else purchases=purchases.map(x=>x.id===id?{...x,status:'Cancelada',cancelledAt:new Date().toISOString()}:x);if(Array.isArray(data.products))products=data.products;if(Array.isArray(data.financial))financialTransactions=data.financial;savePurchasesLocal();localStorage.setItem('mj_products',JSON.stringify(products));localStorage.setItem('mj_financial_transactions',JSON.stringify(financialTransactions));closePurchaseCancelConfirm();closePurchaseModal();renderPurchases();renderProducts();renderStock();renderFinance();renderDashboardLowStock();toast(`${p.num} cancelada. Estoque e financeiro estornados.`);go('compras');}catch(err){console.error('[MJ] Cancelamento compra:',err);toast(err.message||'Não foi possível cancelar a compra.');}finally{if(btn){btn.disabled=false;btn.textContent='Sim, cancelar compra';}}}
async function refreshPurchasesModuleData(){
  if(!window.MJCloud?.refreshPurchasesContext)return;
  try{
    const data=await window.MJCloud.refreshPurchasesContext();
    ...
    renderPurchases();populatePurchaseSuppliers();populatePurchaseProducts();renderProducts();renderStock();
  }catch(err){console.error('[MJ] Atualização compras:',err);toast(err.message||'Não foi possível atualizar Compras.');}
}

function initPurchases(){
  bindReceiptInput('purchaseReceiptFile','purchaseReceiptFileName','purchase');
  document.getElementById('newPurchaseBtn')?.addEventListener('click',openPurchaseForm);document.getElementById('closePurchaseBtn')?.addEventListener('click',closePurchaseForm);document.getElementById('cancelPurchaseBtn')?.addEventListener('click',closePurchaseForm);document.getElementById('addPurchaseItemBtn')?.addEventListener('click',addPurchaseItem);document.getElementById('savePurchaseBtn')?.addEventListener('click',finalizePurchase);
  document.getElementById('purchaseProductSelect')?.addEventListener('change',e=>{const p=products.find(x=>x.id===e.target.value),inp=document.getElementById('purchaseItemCost');if(inp)inp.value=p?Number(p.cost||0).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}):'';});
  ['purchaseDiscount','purchaseFreight'].forEach(id=>document.getElementById(id)?.addEventListener('input',purchaseCalc));document.getElementById('purchaseInstallments')?.addEventListener('change',e=>{if(Number(e.target.value)>1)document.getElementById('purchaseFinanceStatus').value='Pendente';});['purchaseSearch','purchaseStatusFilter','purchaseFinanceFilter','purchaseMonthFilter'].forEach(id=>document.getElementById(id)?.addEventListener(id==='purchaseSearch'?'input':'change',renderPurchases));document.getElementById('clearPurchaseFilters')?.addEventListener('click',()=>{document.getElementById('purchaseSearch').value='';document.getElementById('purchaseStatusFilter').value='';document.getElementById('purchaseFinanceFilter').value='';document.getElementById('purchaseMonthFilter').value='';renderPurchases();});
  document.querySelectorAll('[data-close-purchase-modal]').forEach(el=>el.addEventListener('click',closePurchaseModal));document.addEventListener('click',e=>{if(e.target.closest('[data-cancel-purchase-dismiss]'))closePurchaseCancelConfirm();if(e.target.closest('#confirmPurchaseCancelBtn'))executePurchaseCancellation();});
  document.querySelector('#nav button[data-view="compras"]')?.addEventListener('click',()=>{renderPurchases();populatePurchaseSuppliers();populatePurchaseProducts();});
  document.querySelector('#nav button[data-view="fornecedores"]')?.addEventListener('click',renderSuppliers);
  populatePurchaseSuppliers();populatePurchaseProducts();renderPurchaseDraft();renderPurchases();
}
window.editSupplier=editSupplier;window.deleteSupplier=deleteSupplier;window.updatePurchaseItem=updatePurchaseItem;window.removePurchaseItem=removePurchaseItem;window.viewPurchase=viewPurchase;window.printPurchase=printPurchase;window.requestCancelPurchase=requestCancelPurchase;
initSuppliers();initPurchases();
...
// V11.15 - Dashboard e estoque exibem somente dados reais do Google Sheets/cache.
try{renderDashboardLowStock();renderDashboardLiveData();}catch(e){console.warn('[MJ V11.15] Dashboard inicial:',e);}


// =====================================================
// MJ ADMIN V11.23 - RELATÓRIOS GERENCIAIS
// =====================================================
let currentReportExport={headers:[],rows:[],filename:'relatorio'};
function reportDateInRange(value,start,end){const d=String(value||'').slice(0,10);return !!d&&(!start||d>=start)&&(!end||d<=end);}
...
function reportMetric(icon,label,value,detail=''){return `<article class="report-metric"><span class="report-metric-icon">${icon}</span><div><small>${esc(label)}</small><strong>${esc(String(value))}</strong>${detail?`<span>${esc(detail)}</span>`:''}</div></article>`;}
function reportEmpty(text='Nenhum dado encontrado para o período selecionado.'){return `<div class="report-empty">${esc(text)}</div>`;}
function reportSetTable(headers,rows,title,subtitle){
  const head=document.getElementById('reportTableHead'),body=document.getElementById('reportTableBody');if(!head||!body)return;
  head.innerHTML=`<tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr>`;
  body.innerHTML=rows.length?rows.map(r=>`<tr>${r.display.map((v,i)=>`<td>${i===0?`<b>${v}</b>`:v}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${Math.max(1,headers.length)}">${reportEmpty()}</td></tr>`;
  document.getElementById('reportTableTitle').textContent=title;document.getElementById('reportTableSubtitle').textContent=subtitle||'';document.getElementById('reportResultCount').textContent=`${rows.length} registro(s)`;
  currentReportExport={headers,rows:rows.map(r=>r.raw),filename:`mj-${(document.getElementById('reportType')?.value||'relatorio')}-${currentISODate()}`};
}
function reportSetBars(title,subtitle,items,valueFormatter=moneyBR){
  document.getElementById('reportChartTitle').textContent=title;document.getElementById('reportChartSubtitle').textContent=subtitle||'';
  const box=document.getElementById('reportChart');if(!box)return;const clean=items.filter(x=>Number(x.value)>0).slice(0,8),max=Math.max(1,...clean.map(x=>Number(x.value)||0));
  box.innerHTML=clean.length?clean.map(x=>`<div class="report-bar-row"><div class="report-bar-label"><b>${esc(x.label)}</b><span>${esc(valueFormatter(Number(x.value)||0))}</span></div><div class="report-bar-track"><i style="width:${Math.max(3,(Number(x.value)||0)/max*100)}%"></i></div></div>`).join(''):reportEmpty();
}
function reportSetRanking(title,subtitle,items,valueFormatter=moneyBR){
  document.getElementById('reportRankingTitle').textContent=title;document.getElementById('reportRankingSubtitle').textContent=subtitle||'';const box=document.getElementById('reportRanking');if(!box)return;
  box.innerHTML=items.length?items.slice(0,7).map((x,i)=>`<div class="report-rank-item"><span class="report-rank-pos">${i+1}</span><div><b>${esc(x.label)}</b><small>${esc(x.detail||'')}</small></div><strong>${esc(valueFormatter(Number(x.value)||0))}</strong></div>`).join(''):reportEmpty();
}
function aggregateBy(list,keyFn,valueFn){const map=new Map();list.forEach(x=>{const k=keyFn(x)||'Não informado';map.set(k,(map.get(k)||0)+(Number(valueFn(x))||0));});return [...map.entries()].map(([label,value])=>({label,value})).sort((a,b)=>b.value-a.value);}
function reportPeriod(){const start=document.getElementById('reportStartDate')?.value||'',end=document.getElementById('reportEndDate')?.value||'';return {start,end};}
function renderReportOverview(start,end){
  const validSales=sales.filter(s=>s.status!=='Cancelado'&&reportDateInRange(s.date,start,end));const saleTotal=validSales.reduce((a,s)=>a+Number(s.total||0),0);
  const fin=financialTransactions.filter(t=>reportDateInRange(t.paidAt||t.date,start,end)&&txStatus(t)==='Pago'),inc=fin.filter(t=>t.type==='Entrada').reduce((a,t)=>a+Number(t.value||0),0),out=fin.filter(t=>t.type==='Saída').reduce((a,t)=>a+Number(t.value||0),0);
  const qs=quotes.filter(q=>reportDateInRange(q.date,start,end)),ps=purchases.filter(p=>p.status!=='Cancelada'&&reportDateInRange(p.date,start,end));const purchaseTotal=ps.reduce((a,p)=>a+Number(p.total||0),0),result=inc-out;
  document.getElementById('reportMetrics').innerHTML=[reportMetric('🛒','Vendas',moneyBR(saleTotal),`${validSales.length} venda(s)`),reportMetric('↗','Recebido',moneyBR(inc)),reportMetric('↘','Pago',moneyBR(out)),reportMetric('$','Resultado',moneyBR(result)),reportMetric('🧾','Orçamentos',String(qs.length))].join('');
  const byMonth=aggregateBy(validSales,s=>String(s.date||'').slice(0,7),s=>s.total).sort((a,b)=>a.label.localeCompare(b.label)).map(x=>({...x,label:reportMonthLabel(x.label)}));reportSetBars('Vendas por mês','Valor das vendas não canceladas no período',byMonth);
  const topClients=aggregateBy(validSales,s=>saleClientName(s),s=>s.total).map(x=>({...x,detail:'Em vendas'}));reportSetRanking('Clientes em destaque','Maiores valores vendidos no período',topClients);
  const rows=[...validSales].sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(s=>({display:[esc(s.num||'-'),brDate(s.date),esc(saleClientName(s)),moneyBR(s.total),esc(s.payment||'-'),`<span class="status ${saleStatusClass(s.status)}">${esc(s.status)}</span>`],raw:[s.num||'',brDate(s.date),saleClientName(s),Number(s.total||0).toFixed(2),s.payment||'',s.status||'']}));reportSetTable(['Venda','Data','Cliente','Valor','Pagamento','Status'],rows,'Movimentação de vendas',`Compras registradas no período: ${ps.length} • ${moneyBR(purchaseTotal)}`);
}
function renderReportSales(start,end){
 const list=sales.filter(s=>s.status!=='Cancelado'&&reportDateInRange(s.date,start,end));const total=list.reduce((a,s)=>a+Number(s.total||0),0),paid=list.filter(s=>s.status==='Pago'),pending=list.filter(s=>s.status!=='Pago');const ticket=list.length?total/list.length:0;
 document.getElementById('reportMetrics').innerHTML=[reportMetric('🛒','Vendas',String(list.length)),reportMetric('$','Faturamento',moneyBR(total)),reportMetric('✓','Pagas',String(paid.length),moneyBR(paid.reduce((a,s)=>a+Number(s.total||0),0))),reportMetric('◷','Pendentes',String(pending.length),moneyBR(pending.reduce((a,s)=>a+Number(s.total||0),0))),reportMetric('Ø','Ticket médio',moneyBR(ticket))].join('');
 reportSetBars('Formas de pagamento','Distribuição do valor vendido',aggregateBy(list,s=>s.payment||'Não informado',s=>s.total));
 const productMap=new Map();list.forEach(s=>(s.items||[]).forEach(i=>{const k=i.name||'Item';const old=productMap.get(k)||{qty:0,value:0};old.qty+=Number(i.qty||0);old.value+=Number(i.qty||0)*Number(i.unitPrice||0);productMap.set(k,old);}));const tops=[...productMap.entries()].map(([label,v])=>({label,value:v.value,detail:`${v.qty.toLocaleString('pt-BR')} un.`})).sort((a,b)=>b.value-a.value);reportSetRanking('Produtos / serviços mais vendidos','Ranking pelo valor vendido',tops);
 const rows=[...list].sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(s=>({display:[esc(s.num||'-'),brDate(s.date),esc(saleClientName(s)),moneyBR(s.total),esc(s.payment||'-'),esc(s.seller||'-'),`<span class="status ${saleStatusClass(s.status)}">${esc(s.status)}</span>`],raw:[s.num||'',brDate(s.date),saleClientName(s),Number(s.total||0).toFixed(2),s.payment||'',s.seller||'',s.status||'']}));reportSetTable(['Venda','Data','Cliente','Valor','Pagamento','Vendedor','Status'],rows,'Vendas do período','Vendas canceladas não entram nos totais.');
}
function renderReportFinance(start,end){
 const list=financialTransactions.filter(t=>reportDateInRange(t.dueDate||t.date,start,end));const paid=list.filter(t=>txStatus(t)==='Pago'),pending=list.filter(t=>txStatus(t)==='Pendente'),incPaid=paid.filter(t=>t.type==='Entrada').reduce((a,t)=>a+Number(t.value||0),0),outPaid=paid.filter(t=>t.type==='Saída').reduce((a,t)=>a+Number(t.value||0),0),rec=pending.filter(t=>t.type==='Entrada').reduce((a,t)=>a+Number(t.value||0),0),pay=pending.filter(t=>t.type==='Saída').reduce((a,t)=>a+Number(t.value||0),0);
 document.getElementById('reportMetrics').innerHTML=[reportMetric('↗','Recebido',moneyBR(incPaid)),reportMetric('↘','Pago',moneyBR(outPaid)),reportMetric('$','Saldo realizado',moneyBR(incPaid-outPaid)),reportMetric('◷','A receber',moneyBR(rec)),reportMetric('◷','A pagar',moneyBR(pay))].join('');
 reportSetBars('Movimentação financeira','Valores realizados por tipo',[{label:'Entradas recebidas',value:incPaid},{label:'Saídas pagas',value:outPaid},{label:'A receber',value:rec},{label:'A pagar',value:pay}]);
 reportSetRanking('Categorias com maior movimento','Entradas e saídas somadas',aggregateBy(list,t=>t.category||'Outros',t=>t.value).map(x=>({...x,detail:'Movimentação total'})));
 const rows=[...list].sort((a,b)=>String(b.dueDate||b.date).localeCompare(String(a.dueDate||a.date))).map(t=>{const st=displayFinanceStatus(t);return {display:[brDate(t.dueDate||t.date),esc(t.description||'-'),esc(t.type),moneyBR(t.value),esc(t.payment||'-'),esc(t.source||'-'),`<span class="status ${financeStatusClass(st)}">${esc(st)}</span>`],raw:[brDate(t.dueDate||t.date),t.description||'',t.type||'',Number(t.value||0).toFixed(2),t.payment||'',t.source||'',st]}});reportSetTable(['Vencimento','Descrição','Tipo','Valor','Pagamento','Origem','Status'],rows,'Contas e movimentações','Considera o vencimento dentro do período selecionado.');
}
function renderReportPurchases(start,end){
 const list=purchases.filter(p=>p.status!=='Cancelada'&&reportDateInRange(p.date,start,end)),total=list.reduce((a,p)=>a+Number(p.total||0),0),avg=list.length?total/list.length:0;
 document.getElementById('reportMetrics').innerHTML=[reportMetric('📦','Compras',String(list.length)),reportMetric('$','Total comprado',moneyBR(total)),reportMetric('Ø','Ticket médio',moneyBR(avg)),reportMetric('🚚','Fornecedores',String(new Set(list.map(p=>purchaseSupplierName(p))).size)),reportMetric('◫','Itens comprados',String(list.reduce((a,p)=>a+(p.items||[]).reduce((s,i)=>s+Number(i.qty||0),0),0).toLocaleString('pt-BR')))].join('');
 reportSetBars('Compras por fornecedor','Valor comprado no período',aggregateBy(list,p=>purchaseSupplierName(p),p=>p.total));
 const itemMap=new Map();list.forEach(p=>(p.items||[]).forEach(i=>{const k=i.name||'Produto';itemMap.set(k,(itemMap.get(k)||0)+Number(i.qty||0));}));reportSetRanking('Produtos mais comprados','Quantidade de entrada no estoque',[...itemMap.entries()].map(([label,value])=>({label,value,detail:'Quantidade'})).sort((a,b)=>b.value-a.value),v=>v.toLocaleString('pt-BR'));
 const rows=[...list].sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(p=>({display:[esc(p.num||'-'),brDate(p.date),esc(purchaseSupplierName(p)),moneyBR(p.total),esc(p.payment||'-'),esc(purchaseFinanceState(p))],raw:[p.num||'',brDate(p.date),purchaseSupplierName(p),Number(p.total||0).toFixed(2),p.payment||'',purchaseFinanceState(p)]}));reportSetTable(['Compra','Data','Fornecedor','Valor','Pagamento','Financeiro'],rows,'Compras do período','Compras canceladas não entram nos totais.');
}
function renderReportProducts(){
 const active=products.filter(p=>p.status==='Ativo'),tracked=active.filter(isStockTracked),stockValue=tracked.reduce((a,p)=>a+Number(p.stock||0)*Number(p.cost||0),0),low=tracked.filter(isLowStock);
 document.getElementById('reportMetrics').innerHTML=[reportMetric('◫','Produtos ativos',String(active.length)),reportMetric('▦','Itens controlados',String(tracked.length)),reportMetric('$','Valor em estoque',moneyBR(stockValue)),reportMetric('!','Estoque baixo',String(low.length)),reportMetric('⚙','Serviços',String(active.filter(p=>p.category==='Serviços').length))].join('');
 reportSetBars('Valor por produto em estoque','Custo × quantidade disponível',tracked.map(p=>({label:p.name,value:Number(p.stock||0)*Number(p.cost||0)})).sort((a,b)=>b.value-a.value));
 reportSetRanking('Atenção de estoque','Itens abaixo ou no estoque mínimo',low.map(p=>({label:p.name,value:Number(p.stock||0),detail:`Mínimo: ${Number(p.minStock||0).toLocaleString('pt-BR')}`})).sort((a,b)=>a.value-b.value),v=>v.toLocaleString('pt-BR'));
 const rows=active.map(p=>({display:[esc(p.code||'-'),esc(p.name||'-'),esc(p.category||'-'),esc(p.unit||'-'),isStockTracked(p)?Number(p.stock||0).toLocaleString('pt-BR'):'—',isStockTracked(p)?Number(p.minStock||0).toLocaleString('pt-BR'):'—',moneyBR(p.cost||0),isVariablePriceProduct(p)?'Variável':moneyBR(p.price||0)],raw:[p.code||'',p.name||'',p.category||'',p.unit||'',isStockTracked(p)?Number(p.stock||0):'',isStockTracked(p)?Number(p.minStock||0):'',Number(p.cost||0).toFixed(2),isVariablePriceProduct(p)?'Variável':Number(p.price||0).toFixed(2)]}));reportSetTable(['Código','Produto','Categoria','Unidade','Estoque','Mínimo','Custo','Venda'],rows,'Produtos e estoque','Este relatório representa a posição atual; o filtro de datas não se aplica.');
}
function renderReportQuotes(start,end){
 const list=quotes.filter(q=>reportDateInRange(q.date,start,end)),total=list.reduce((a,q)=>a+quoteTotal(q),0),approved=list.filter(q=>q.status==='Aprovado'),converted=list.filter(q=>q.status==='Convertido'),rate=list.length?converted.length/list.length*100:0;
 document.getElementById('reportMetrics').innerHTML=[reportMetric('🧾','Orçamentos',String(list.length)),reportMetric('$','Valor orçado',moneyBR(total)),reportMetric('✓','Aprovados',String(approved.length)),reportMetric('↗','Convertidos',String(converted.length)),reportMetric('%','Conversão',`${rate.toLocaleString('pt-BR',{maximumFractionDigits:1})}%`)].join('');
 reportSetBars('Orçamentos por status','Valor total por situação',aggregateBy(list,q=>q.status||'Pendente',q=>quoteTotal(q)));
 reportSetRanking('Clientes mais orçados','Valor acumulado dos orçamentos',aggregateBy(list,q=>quoteClientName(q),q=>quoteTotal(q)).map(x=>({...x,detail:'Valor orçado'})));
 const rows=[...list].sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(q=>({display:[esc(q.num||'-'),brDate(q.date),esc(quoteClientName(q)),moneyBR(quoteTotal(q)),brDate(q.validity),`<span class="status ${statusClass(q.status)}">${esc(q.status)}</span>`],raw:[q.num||'',brDate(q.date),quoteClientName(q),quoteTotal(q).toFixed(2),brDate(q.validity),q.status||'']}));reportSetTable(['Orçamento','Data','Cliente','Valor','Validade','Status'],rows,'Orçamentos do período','Conversão considera os orçamentos com status Convertido.');
}
function renderReportClients(start,end){
 const list=clients.filter(c=>c.status==='Ativo'),periodSales=sales.filter(s=>s.status!=='Cancelado'&&reportDateInRange(s.date,start,end));const totals=new Map();periodSales.forEach(s=>totals.set(s.clientId||saleClientName(s),(totals.get(s.clientId||saleClientName(s))||0)+Number(s.total||0)));
 const withSales=list.filter(c=>totals.has(c.id)),without=list.length-withSales.length;document.getElementById('reportMetrics').innerHTML=[reportMetric('👥','Clientes ativos',String(list.length)),reportMetric('🛒','Compraram no período',String(withSales.length)),reportMetric('○','Sem compra no período',String(without)),reportMetric('$','Valor vendido',moneyBR(periodSales.reduce((a,s)=>a+Number(s.total||0),0))),reportMetric('＋','Cadastros totais',String(clients.length))].join('');
 const ranks=list.map(c=>({label:clientDisplayName(c),value:totals.get(c.id)||0,detail:c.type==='PJ'?'Pessoa Jurídica':'Pessoa Física'})).sort((a,b)=>b.value-a.value);reportSetBars('Vendas por cliente','Clientes com movimentação no período',ranks.filter(x=>x.value>0));reportSetRanking('Maiores clientes','Ranking pelo valor vendido',ranks.filter(x=>x.value>0));
 const rows=list.map(c=>({display:[esc(clientDisplayName(c)),esc(c.type||'-'),esc(c.document||'-'),esc(c.phone||'-'),esc(c.email||'-'),moneyBR(totals.get(c.id)||0)],raw:[clientDisplayName(c),c.type||'',c.document||'',c.phone||'',c.email||'',Number(totals.get(c.id)||0).toFixed(2)]}));reportSetTable(['Cliente','Tipo','Documento','Telefone','E-mail','Vendas no período'],rows,'Clientes ativos','Valor de vendas considera o período selecionado.');
}
function renderReports(){
 const {start,end}=reportPeriod();if(start&&end&&start>end){toast('A data inicial não pode ser maior que a data final.');return;}
 const type=document.getElementById('reportType')?.value||'overview',hint=document.getElementById('reportPeriodHint');if(hint)hint.textContent=type==='products'?'Produtos / Estoque mostra a posição atual e ignora o período.':`Período: ${start?brDate(start):'início'} até ${end?brDate(end):'hoje'}`;
 if(type==='sales')renderReportSales(start,end);else if(type==='finance')renderReportFinance(start,end);else if(type==='purchases')renderReportPurchases(start,end);else if(type==='products')renderReportProducts();else if(type==='quotes')renderReportQuotes(start,end);else if(type==='clients')renderReportClients(start,end);else renderReportOverview(start,end);
}
function csvCell(v){const s=String(v??'').replace(/"/g,'""');return `"${s}"`;}
function exportCurrentReport(){if(!currentReportExport.headers.length){toast('Gere o relatório antes de exportar.');return;}const lines=[currentReportExport.headers,...currentReportExport.rows].map(r=>r.map(csvCell).join(';'));const blob=new Blob(['\ufeff'+lines.join('\r\n')],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${currentReportExport.filename}.csv`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),500);toast('Relatório exportado em CSV.');}
function printCurrentReport(){const section=document.getElementById('relatorios');if(!section)return;document.body.classList.add('printing-report');window.print();setTimeout(()=>document.body.classList.remove('printing-report'),500);}
function initReportsV1122(){const start=document.getElementById('reportStartDate'),end=document.getElementById('reportEndDate');if(!start||!end)return;const now=new Date(),first=new Date(now.getFullYear(),now.getMonth(),1);start.value=`${first.getFullYear()}-${String(first.getMonth()+1).padStart(2,'0')}-01`;end.value=currentISODate();document.getElementById('refreshReportBtn')?.addEventListener('click',renderReports);document.getElementById('reportType')?.addEventListener('change',renderReports);start.addEventListener('change',renderReports);end.addEventListener('change',renderReports);document.getElementById('exportReportBtn')?.addEventListener('click',exportCurrentReport);document.getElementById('printReportBtn')?.addEventListener('click',printCurrentReport);document.querySelector('#nav button[data-view="relatorios"]')?.addEventListener('click',()=>setTimeout(renderReports,0));renderReports();}
initReportsV1122();

/* =====================================================
   V11.23 - USUÁRIOS, LOGIN E PERMISSÕES
   ===================================================== */
...
const ROLE_DEFAULT_PERMISSIONS = {
  ...
  Gerente:['dashboard','nova-venda','vendas','orcamentos','produtos','clientes','fornecedores','compras','estoque','financeiro','despesas','relatorios','configuracoes'],
  Vendedor:['dashboard','nova-venda','vendas','orcamentos','clientes','produtos'],
  Financeiro:['dashboard','vendas','compras','financeiro','despesas','relatorios','clientes','fornecedores'],
  Estoque:['dashboard','produtos','fornecedores','compras','estoque']
};
let systemUsers=[];

function applyCurrentUserUI(){
  const u=MJCurrentUser||{};
  const name=u.name||'Usuário';
  const first=name.trim().split(/\s+/)[0]||'Usuário';
  const initials=name.trim().split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()||'U';
  const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
  set('welcomeTitle',`Olá, ${first}! 👋`);set('topUserName',name);set('topUserRole',u.role||'Usuário');set('topUserAvatar',initials);
  const seller=document.getElementById('saleSeller');if(seller&&!seller.value.trim())seller.value=name;

  document.querySelectorAll('#nav button[data-view]').forEach(btn=>{
    const view=btn.dataset.view;
    btn.hidden=!canAccess(view);
  });
  document.querySelectorAll('[data-goto]').forEach(el=>{
    if(!canAccess(el.dataset.goto))el.hidden=true;
  });

  const admin=u.role==='Administrador';
  document.querySelector('[data-settings-tab="usuarios"]')?.toggleAttribute('hidden',!admin);
  document.querySelector('[data-settings-tab="integracoes"]')?.toggleAttribute('hidden',!admin);
  const usersPanel=document.querySelector('[data-settings-panel="usuarios"]');if(usersPanel&&!admin)usersPanel.classList.remove('active');
  const integrationPanel=document.querySelector('[data-settings-panel="integracoes"]');if(integrationPanel&&!admin)integrationPanel.classList.remove('active');
  if(!admin){
    document.querySelectorAll('#settingsTabs button').forEach(b=>b.classList.toggle('active',b.dataset.settingsTab==='seguranca'));
    document.querySelector('[data-settings-panel="seguranca"]')?.classList.add('active');
  }
}

function initSettingsTabs(){
  document.querySelectorAll('#settingsTabs [data-settings-tab]').forEach(btn=>btn.addEventListener('click',()=>{
    if(btn.hidden)return;
    const tab=btn.dataset.settingsTab;
    document.querySelectorAll('#settingsTabs [data-settings-tab]').forEach(b=>b.classList.toggle('active',b===btn));
    document.querySelectorAll('[data-settings-panel]').forEach(p=>p.classList.toggle('active',p.dataset.settingsPanel===tab));
    if(tab==='usuarios'&&MJCurrentUser.role==='Administrador')loadUsers();
  }));
}

function formatDateTimeBR(v){
  if(!v)return 'Nunca';
  const d=new Date(v);return Number.isNaN(d.getTime())?'Nunca':d.toLocaleString('pt-BR');
}
function renderUsers(){
  const body=document.getElementById('usersBody');if(!body)return;
  if(!systemUsers.length){body.innerHTML='<tr><td colspan="6"><div class="empty-note">Nenhum usuário cadastrado.</div></td></tr>';return;}
  body.innerHTML=systemUsers.map(u=>`<tr><td><b>${esc(u.name||'-')}</b></td><td>${esc(u.email||'-')}</td><td><span class="user-role-pill">${esc(u.role||'-')}</span></td><td><span class="status ${u.status==='Ativo'?'paid':'rejected'}">${esc(u.status||'Ativo')}</span></td><td>${esc(formatDateTimeBR(u.lastLoginAt))}</td><td><div class="sale-row-actions"><button class="table-action" type="button" onclick="openUserModal('${u.id}')">Editar</button>${u.id!==MJCurrentUser.id?`<button class="table-action danger" type="button" onclick="deleteSystemUser('${u.id}')">Excluir</button>`:''}</div></td></tr>`).join('');
}
async function loadUsers(){
  if(MJCurrentUser.role!=='Administrador')return;
  const body=document.getElementById('usersBody');if(body)body.innerHTML='<tr><td colspan="6"><div class="empty-note">Carregando usuários...</div></td></tr>';
  try{const res=await window.MJCloud.listUsers();systemUsers=Array.isArray(res.data)?res.data:[];renderUsers();}
  catch(err){if(body)body.innerHTML=`<tr><td colspan="6"><div class="empty-note">${esc(err.message||'Não foi possível carregar os usuários.')}</div></td></tr>`;}
}
function renderPermissionChecks(selected){
  const wrap=document.getElementById('userPermissions');if(!wrap)return;
  const set=new Set(selected||[]);
  wrap.innerHTML=Object.entries(USER_PERMISSION_LABELS).map(([key,label])=>`<label class="permission-item"><input type="checkbox" value="${key}" ${set.has(key)?'checked':''}><span><b>${esc(label)}</b></span></label>`).join('');
}
function selectedPermissions(){return [...document.querySelectorAll('#userPermissions input[type="checkbox"]:checked')].map(x=>x.value);}
function openUserModal(id=''){
  if(MJCurrentUser.role!=='Administrador')return toast('Somente administradores podem gerenciar usuários.');
  ...
  document.getElementById('userId').value=u?.id||'';document.getElementById('userName').value=u?.name||'';document.getElementById('userEmail').value=u?.email||'';
  document.getElementById('userRole').value=u?.role||'Vendedor';document.getElementById('userStatus').value=u?.status||'Ativo';document.getElementById('userPassword').value='';
  const help=document.getElementById('userPasswordHelp');if(help)help.textContent=u?'Deixe em branco para manter a senha atual.':'Mínimo de 6 caracteres.';
  renderPermissionChecks(u?.permissions||ROLE_DEFAULT_PERMISSIONS[u?.role||'Vendedor']||[]);
  const modal=document.getElementById('userModal');modal?.classList.add('show');modal?.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';
}
function closeUserModal(){const modal=document.getElementById('userModal');modal?.classList.remove('show');modal?.setAttribute('aria-hidden','true');document.body.style.overflow='';}
async function saveSystemUser(){
  const id=document.getElementById('userId').value;const password=document.getElementById('userPassword').value;
  const usuario={id,name:document.getElementById('userName').value.trim(),email:document.getElementById('userEmail').value.trim(),role:document.getElementById('userRole').value,status:document.getElementById('userStatus').value,permissions:selectedPermissions(),password};
  if(!usuario.name||!usuario.email){toast('Informe nome e e-mail.');return;}if(!id&&password.length<6){toast('A senha inicial deve ter pelo menos 6 caracteres.');return;}
  const btn=document.getElementById('saveUserBtn');if(btn){btn.disabled=true;btn.textContent='Salvando...';}
  try{await window.MJCloud.saveUser(usuario);closeUserModal();toast(id?'Usuário atualizado.':'Usuário criado.');await loadUsers();}
  catch(err){toast(err.message||'Não foi possível salvar o usuário.');}
  finally{if(btn){btn.disabled=false;btn.textContent='Salvar usuário';}}
}
async function deleteSystemUser(id){
  const u=systemUsers.find(x=>String(x.id)===String(id));if(!u)return;
  if(!confirm(`Excluir o usuário ${u.name}?`))return;
  try{await window.MJCloud.deleteUser(id);toast('Usuário excluído.');await loadUsers();}catch(err){toast(err.message||'Não foi possível excluir o usuário.');}
}
async function changeMyPassword(){
  const current=document.getElementById('currentPassword').value,newPass=document.getElementById('newPassword').value,confirmPass=document.getElementById('confirmNewPassword').value;
  if(!current||!newPass){toast('Informe a senha atual e a nova senha.');return;}if(newPass.length<6){toast('A nova senha deve ter pelo menos 6 caracteres.');return;}if(newPass!==confirmPass){toast('A confirmação da nova senha não confere.');return;}
  const btn=document.getElementById('changePasswordBtn');if(btn){btn.disabled=true;btn.textContent='Alterando...';}
  try{await window.MJCloud.changeMyPassword(current,newPass);document.getElementById('currentPassword').value='';document.getElementById('newPassword').value='';document.getElementById('confirmNewPassword').value='';toast('Senha alterada com sucesso.');}
