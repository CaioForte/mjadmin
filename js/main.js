...
function purchaseFinanceState(p){
  ...
  const tx=financialTransactions.find(t=>t.source==='Compra'&&String(t.sourceId)===String(p.id));
  return tx?.status||p.financeStatus||'Pago';
}
...
function openPurchaseForm(){const p=document.getElementById('purchaseFormPanel');if(!p)return;purchaseItemsDraft=[];p.hidden=false;document.getElementById('purchaseNumber').value=nextPurchaseNumber();document.getElementById('purchaseDate').value=currentISODate();document.getElementById('purchaseDueDate').value=currentISODate();document.getElementById('purchaseSupplier').value='';document.getElementById('purchasePayment').value='PIX';document.getElementById('purchaseFinanceStatus').value='Pago';document.getElementById('purchaseGenerateFinance').checked=true;document.getElementById('purchaseUpdateCost').checked=true;document.getElementById('purchaseNotes').value='';document.getElementById('purchaseDiscount').value='0,00';document.getElementById('purchaseFreight').value='0,00';populatePurchaseSuppliers();populatePurchaseProducts();renderPurchaseDraft();p.scrollIntoView({behavior:'smooth',block:'start'});}
function closePurchaseForm(){const p=document.getElementById('purchaseFormPanel');if(p)p.hidden=true;purchaseItemsDraft=[];}
...
async function finalizePurchase(){const supplierId=document.getElementById('purchaseSupplier')?.value,supplier=suppliers.find(s=>s.id===supplierId);if(!supplier){toast('Selecione o fornecedor.');return;}if(!purchaseItemsDraft.length){toast('Adicione pelo menos um produto à compra.');return;}const calc=purchaseCalc();if(parseMoneyBR(document.getElementById('purchaseDiscount').value)>calc.subtotal){toast('O desconto não pode ser maior que o subtotal.');return;}const btn=document.getElementById('savePurchaseBtn');if(btn){btn.disabled=true;btn.textContent='Registrando...';}const compra={id:`cmp-${Date.now()}`,num:document.getElementById('purchaseNumber').value||nextPurchaseNumber(),date:document.getElementById('purchaseDate').value||currentISODate(),supplierId,supplierName:supplier.name,items:purchaseItemsDraft.map(i=>({...i})),subtotal:calc.subtotal,discount:calc.discount,freight:calc.freight,total:calc.total,payment:document.getElementById('purchasePayment').value,financeStatus:document.getElementById('purchaseFinanceStatus').value,generateFinance:document.getElementById('purchaseGenerateFinance').checked,updateCost:document.getElementById('purchaseUpdateCost').checked,dueDate:document.getElementById('purchaseDueDate').value||document.getElementById('purchaseDate').value||currentISODate(),notes:document.getElementById('purchaseNotes').value.trim(),status:'Ativa',createdAt:new Date().toISOString()};try{const response=await window.MJCloud.finalizePurchase(compra),data=response.data||{};const saved=data.compra||compra;if(!purchases.some(p=>p.id===saved.id))purchases.unshift(saved);if(Array.isArray(data.products))products=data.products;if(Array.isArray(data.financial))financialTransactions=data.financial;savePurchasesLocal();localStorage.setItem('mj_products',JSON.stringify(products));localStorage.setItem('mj_financial_transactions',JSON.stringify(financialTransactions));closePurchaseForm();renderPurchases();renderProducts();renderStock();renderFinance();renderDashboardLowStock();populatePurchaseProducts();toast(`${saved.num} registrada. Estoque atualizado.`);go('compras');}catch(err){console.error('[MJ] Compra:',err);toast(err.message||'Não foi possível registrar a compra.');}finally{if(btn){btn.disabled=false;btn.textContent='✓ Registrar compra e entrada';}}}
...
function initPurchases(){
  ...
  ['purchaseDiscount','purchaseFreight'].forEach(id=>document.getElementById(id)?.addEventListener('input',purchaseCalc));['purchaseSearch','purchaseStatusFilter','purchaseFinanceFilter','purchaseMonthFilter'].forEach(id=>document.getElementById(id)?.addEventListener(id==='purchaseSearch'?'input':'change',renderPurchases));document.getElementById('clearPurchaseFilters')?.addEventListener('click',()=>{document.getElementById('purchaseSearch').value='';document.getElementById('purchaseStatusFilter').value='';document.getElementById('purchaseFinanceFilter').value='';document.getElementById('purchaseMonthFilter').value='';renderPurchases();});
