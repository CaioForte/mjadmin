...
function salvarDespesa_(despesa,auditAction){
  ...
  return withLock_(function(){
    ...
    if(idx>=0)all[idx]=ft;else all.unshift(ft); writeObjects_('Financeiro',all); log_(auditAction||'salvarDespesa',despesa.id,despesa.description||''); return {despesa:despesa,financeiro:ft};
  });
}
...
function marcarDespesaPaga_(id){ const d=readObjects_('Despesas').find(x=>String(x.id)===String(id)); if(!d)throw new Error('Despesa não encontrada.'); d.status='Pago'; return salvarDespesa_(d,'marcarDespesaPaga'); }
